...
require_once DOL_DOCUMENT_ROOT . '/compta/paiement/especes/class/remiseespeces.class.php';
...
$result = restrictedArea ( $user, 'banque', '', '' ); // A vérifier
...
$object = new RemiseEspeces ( $db );

/*
 * Actions
 */

if ($action == 'set_note' && $user->rights->banque->cheque) {
	...
	if ($result > 0) {
		...
		$result = $object->set_note ( $user, $_POST ['note'] );
		if ($result < 0) {
...
if ($action == 'set_type_cash' && $user->rights->banque->cheque && $_POST ['type_cash'] != null) {
	...
	if ($result > 0) {
		...
		$result = $object->set_type_cash ( $user, $_POST ['type_cash'], $_POST ['nombre'] );
		if ($result < 0) {
			$mesg = '<div class="error">' . $object->error . '</div>';
		}
	} else {
...
if ($action == 'new') {
	...
	$head [$h] [1] = $langs->trans ( "MenuCashDeposits" );
	$hselected = $h;
	$h ++;
	
	print_fiche_titre ( $langs->trans ( "Cash" ) );
} else {
	...
	$head [$h] [1] = $langs->trans ( "CashReceipt" );
	$hselected = $h;
	$h ++;
	...
	dol_fiche_head ( $head, $hselected, $langs->trans ( "Cash" ), 0, 'payment' );
	
	/*
	 * Confirmation de la suppression du bordereau
	 */
	if ($action == 'delete') {
		$ret = $form->form_confirm ( $_SERVER ["PHP_SELF"] . '?id=' . $object->id, $langs->trans ( "DeleteCashReceipt" ), $langs->trans ( "ConfirmDeleteCashReceipt" ), 'confirm_delete', '', '', 1 );
		if ($ret == 'html')
			print '<br>';
	}
	 ...
	 */
	if ($action == 'valide') {
		$ret = $form->form_confirm ( $_SERVER ["PHP_SELF"] . '?id=' . $object->id, $langs->trans ( "ValidateCashReceipt" ), $langs->trans ( "ConfirmValidateCashReceipt" ), 'confirm_valide', '', '', 1 );
		if ($ret == 'html')
			print '<br>';
	}
}
...
if ($action == 'new') {
	...
	print $langs->trans ( "SelectCashTransactionAndGenerate" ) . '<br><br>' . "\n";
	...
	// Filter
	print '<tr><td width="200">' . $langs->trans ( "DateCashReceived" ) . '</td><td>';
	...
	$sql .= " b.amount, ba.label, b.emetteur, b.num_chq, b.banque, b.label as lelabel";
	...
	if ($resql) {
		...
		while ( $obj = $db->fetch_object ( $resql ) ) {
			...
			$lines [$obj->bid] [$i] ["label"] = $obj->lelabel;
			$i ++;
		}
		
		if ($i == 0) {
			print '<b>' . $langs->trans ( "NoWaitingCash" ) . '</b><br>';
		}
	}
	...
	foreach ( $accounts as $bid => $account_label ) {
		...
		print '<td style="min-width: 120px">' . $langs->trans ( "DateCashReceived" ) . ' ';
		print "</td>\n";
		print '<td style="min-width: 120px">' . $langs->trans ( "RefNumber" ) . "</td>\n";
		print '<td style="min-width: 200px">' . $langs->trans ( "CashType" ) . "</td>\n";
	...
	// Nb of cheques
	print '<tr><td>' . $langs->trans ( 'NbOfCash' ) . '</td><td colspan="2">';
	print $object->nbcash;
	print '</td></tr>';
	...
	print '</td>';
	if ($action != 'edit_note')
		print '<td align="right"><a href="' . $_SERVER ["PHP_SELF"] . '?action=edit_note&amp;id=' . $object->id . '">' . img_edit ( $langs->trans ( 'Note' ), 1 ) . '</a></td>';
	print '</tr></table>';
	
	print '</td><td colspan="2">';
	if ($action == 'edit_note') // pas sur
{
		print '<form name="set_note" action="' . $_SERVER ["PHP_SELF"] . '?id=' . $object->id . '" method="post">';
		print '<input type="hidden" name="token" value="' . $_SESSION ['newtoken'] . '">';
		print '<input type="hidden" name="action" value="set_note">';
		print '<input type="text" name="note" value="' . $object->note . '">';
		print '<input type="submit" class="button" value="' . $langs->trans ( 'Modify' ) . '">';
		print '</form>';
	} else {
		print $object->note;
	...
	if ($object->somme_cash == $object->amount) {
		print '<tr><td>' . $langs->trans ( 'Total_cash' ) . '</td><td colspan="2"  style="color: #008000;">';
	} else {
		print '<tr><td>' . $langs->trans ( 'Total_cash' ) . '</td><td colspan="2" style="color: #F00;">';
	}
	print price ( $object->somme_cash );
	print '</td></tr>';
	...
	foreach ( $types_cash as $types ) {
		
		print '<tr><td>';
		
		print '<table class="nobordernopadding" width="100%"><tr><td>';
		print $langs->trans ( $types );
		print '</td>';
		if ($action != 'edit_type_cash')
			print '<td align="right"><a href="' . $_SERVER ["PHP_SELF"] . '?action=edit_type_cash&type=' . $types . '&amp;id=' . $object->id . '">' . img_edit ( $langs->trans ( 'SetDate' ), 1 ) . '</a></td>';
		print '</tr></table>';
		
		print '</td><td colspan="2">';
		if ($action == 'edit_type_cash' && $type == $types) // pas sur
{
			print '<form name="set_type_cash" action="' . $_SERVER ["PHP_SELF"] . '?id=' . $object->id . '&type=' . $types . '" method="post">';
			print '<input type="hidden" name="token" value="' . $_SESSION ['newtoken'] . '">';
			print '<input type="hidden" name="action" value="set_type_cash">';
			print '<input type="hidden" name="type_cash" value="' . $types . '">';
			print '<input type="text" name="nombre" value="' . $object->$types . '">';
			print '<input type="submit" class="button" value="' . $langs->trans ( 'Modify' ) . '">';
			print '</form>';
		} else {
			print $object->$types;
	...
	$sql .= " b.dateo as date, b.datec as datec, b.banque, b.label,";
	...
	if ($resql) {
		...
		$param = "&amp;id=" . $object->id;
		print '<tr class="liste_titre">';
		print_liste_field_titre ( $langs->trans ( "Ecriture" ), '', '', '', '', 'width="30"' );
		print_liste_field_titre ( $langs->trans ( "DateEcriture" ), $_SERVER ["PHP_SELF"], "b.dateo,b.rowid", "", $param, 'align="center"', $sortfield, $sortorder );
		print_liste_field_titre ( $langs->trans ( "Numero" ), $_SERVER ["PHP_SELF"], "b.num_chq", "", $param, 'align="center"', $sortfield, $sortorder );
		print_liste_field_titre ( $langs->trans ( "Type" ), $_SERVER ["PHP_SELF"], "b.label", "", $param, "", $sortfield, $sortorder );
