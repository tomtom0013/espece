 ...
 *
 * @param string $head
 *        	Optionnal head lines
 * @param string $title
 *        	HTML title
 * @param string $help_url
 *        	Link to online url help to show on left menu
 * @param string $target
 *        	Force target on menu links
 * @param int $disablejs
 *        	More content into html header
 * @param int $disablehead
 *        	More content into html header
...
function llxHeader($head = '', $title = '', $help_url = '', $target = '', $disablejs = 0, $disablehead = 0, $arrayofjs = '', $arrayofcss = '') {
	...
	if ($user->rights->banque->lire) {
		...
		if ($resql) {
			...
			if ($numr > 0)
				$menu->add ( '/compta/bank/index.php', $langs->trans ( "BankAccounts" ), 0, $user->rights->banque->lire );
